...
it('can create a visit with refer information', function () {
    $visit = CreateVisitForLink::run(link: $this->link, referer: 'https://example.com/foo/bar?query=string#fragment');
...
it('increments the links total visits when a visit is created', function () {
    ...
    CreateVisitForLink::run($this->link);
...
it('gracefully handles invalid referer URLs', function () {
    $visit = CreateVisitForLink::run(link: $this->link, referer: 'invalid-url');

    expect($visit->referer_host)->toBeNull()
        ->and($visit->referer_path)->toBeNull();
});

it('gracefully handles invalid user agent strings', function () {
    $visit = CreateVisitForLink::run($this->link, 'invalid-user-agent');
