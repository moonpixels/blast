...
it('switches the users current team', function () {
    $this->put(route('user.user.current-team.update'), ['team_id' => $this->team->id])
...
it('does not switch the users current team if they are not a member', function () {
    ...
    $this->put(route('user.user.current-team.update'), ['team_id' => $team->id])
...
it('does not switch the users current team if it does not exist', function () {
    $this->put(route('user.user.current-team.update'), ['team_id' => 2])
        ->assertInvalid('team_id');

    $this->assertNotEquals(2, $this->user->current_team_id);

    $this->put(route('user.user.current-team.update'), ['team_id' => 'foo'])
        ->assertInvalid('team_id');
    ...
    $this->put(route('user.user.current-team.update'), ['team_id' => $ulid])
        ->assertInvalid('team_id');
